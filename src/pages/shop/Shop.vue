<script setup>
	...
	async function lodeMore() {
		skipCount.value += 30
		const url = `https://dummyjson.com/products?limit=30&skip=${skipCount.value}`
		const res = await axios.get(url)
		if (200 == res.status) {
			res.data.products.forEach((product) => {
				...
				<div class="row">
					...
					<div class="col-12 col-lg-9 px-3">
									...
									<div class="overflow-hidden position-relative">
										...
										<div class="hidden card-img-overlay rounded-0 d-flex p-0 bg-opacity-0 justify-content-end align-items-end bg-dark hover-bg-opacity-10 transition-1000">
											<div class="position-absolute top- bottom-0 end-0 btn-group-vertical transform-rotateY-90 hover-transform-rotateY-0 transition-500" style="z-index: 100">
												...
												<!-- <button @click="wishlist.toggleWishlist(product)" class="btn text-light p-1 border-0 rounded-0" :class="wishlist.wishlistStatus(product) ? 'bg-success' : 'bg-secondary'" style="height: 30px; width: 30px">
						...
						<div v-if="products.length > 0" class="text-center">
							...
							<button @click="lodeMore()" class="fw-bold text-white bg-dark hover-border-red hover-text-red hover-bg-transparent btn rounded-0 text-uppercase box-shadow-none" type="button"><small>load more...</small></button>
						</div>
					</div>
				</div>
			</div>
