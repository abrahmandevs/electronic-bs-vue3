<script setup>
	...
	watch(
		...
		async () => {
			await axios
				.get(`https://dummyjson.com/products/category/${productData["category"]}`)
				// .get("http://127.0.0.1:8000/api/products")
				.then((res) => {
					// products.value = res.data
					recomandedProducts.value = res.data.products
				})
...
<template>
	<AppPublicLayout>
		...
		<section class="py-4 px-3">
			...
			<div class="container-lg">
				<Swiper name="mySwiper" duration="3000" class="z-2 h-100" btnClass="text-white bg-secondary">
					<SwiperSlide v-for="(product, index) in recomandedProducts" :key="index" class="h-100">
						<div class="z-3 card rounded-0 group-hover p-2 h-100">
