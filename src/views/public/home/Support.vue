<script setup>
	const baseUrl = window.location.origin
</script>
<template>
	<div id="home-support " class="my-3">
		<!--support-section-->
		<div class="container">
			<div class="row">
				<div class="col-6 col-lg-3 py-4">
					<div class="d-flex flex-column flex-sm-row align-items-center text-ms-center">
						<img class="d-block p-2 p-sm-0 me-2" :src="`${baseUrl}/assets/images/demo/feature-1.png`" alt="image" />
						<div class="">
							<p class="mb-0 fw-bold">Delivery in 24H</p>
							<p class="mb-0 text-secondary small">Free shipping over $100</p>
						</div>
					</div>
				</div>
				<div class="col-6 col-lg-3 py-4">
					<div class="d-flex flex-column flex-sm-row align-items-center text-ms-center">
						<img class="d-block p-2 p-sm-0 me-2" :src="`${baseUrl}/assets/images/demo/feature-2.png`" alt="image" />
						<div class="">
							<p class="mb-0 fw-bold">24 hours return</p>
							<p class="mb-0 text-secondary small">Free shipping over $100</p>
						</div>
					</div>
				</div>
				<div class="col-6 col-lg-3 py-4">
					<div class="d-flex flex-column flex-sm-row align-items-center text-ms-center">
						<img class="d-block p-2 p-sm-0 me-2" :src="`${baseUrl}/assets/images/demo/feature-3.png`" alt="image" />
						<div class="">
							<p class="mb-0 fw-bold">quality guarantee</p>
							<p class="mb-0 text-secondary small">Quality check by our team</p>
						</div>
					</div>
				</div>
				<div class="col-6 col-lg-3 py-4">
					<div class="d-flex flex-column flex-sm-row align-items-center text-ms-center">
						<img class="d-block p-2 p-sm-0 me-2" :src="`${baseUrl}/assets/images/demo/feature-4.png`" alt="image" />
						<div class="">
							<p class="mb-0 fw-bold">support 24/7</p>
							<p class="mb-0 text-secondary small">Shop with an expert</p>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</template>
